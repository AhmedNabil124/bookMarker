...
function searchWebsite() {
  var searchWebsite = searchUrl.value;
  var temp = "";
  for (let i = 0; i < listWebsites.length; i++) {
    if (
      listWebsites[i].name.toLowerCase().includes(searchWebsite.toLowerCase())
    ) {
      temp += `
    <div class="col-md-12 content">
    <h3 class="d-inline-block">${listWebsites[i].name}</h3>
    <a href="${listWebsites[i].url}" id="visitBtn" class="btn btn-outline-primary d-inline-block mx-2">Visit</a>
    <button  id="deleteBtn" class="btn btn-outline-danger d-inline-block mx-2" onclick="deleteWebsite(${i})">Delete</button>
  </div>
    `;
    }
